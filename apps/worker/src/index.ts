...
const localCorsOrigins = new Set(['http://127.0.0.1:5173', 'http://localhost:5173'])
...
function resolveCorsOrigin(request: Request, env?: Env): string | null {
  ...
  if (localCorsOrigins.has(origin) || parseAllowedOrigins(env).has(origin) || isCloudflarePagesOrigin(origin)) {
    return origin
  }
